...
class robot:
    ...
    def __init__(self, robot_name, ros_namespace = '/dvrk/'):
        ...
        self.__analog_input_pos_si = []
        self.__velocity_joint_current = []
        self.__effort_joint_current = []
        self.__position_cartesian_current = Frame()
        self.__impedance_effort = [] 
        self.__fitted_plane = []
        self.__trocar_wrench = []
        self.__foot_pedal_camera_state = []
        ...
        self.set_wrench_spatial_publisher = rospy.Publisher(full_ros_namespace + '/set_wrench_spatial',
                                                            Wrench, latch=True, queue_size = 1)
        self.set_effort_joint_publisher = rospy.Publisher(full_ros_namespace + '/set_effort_joint',
                                                            JointState, latch=True, queue_size = 1)
        self.set_position_impedance_publisher = rospy.Publisher(full_ros_namespace + '/set_position_impedance',
                                                                Pose, latch=True, queue_size = 1)        
        self.set_stiffness_impedance_publisher = rospy.Publisher(full_ros_namespace + '/set_stiffness_impedance',
                                                        Float32, latch=True, queue_size = 1) 
		...
		#Subscriber per ros-io (potentiometer)	
        rospy.Subscriber(full_ros_namespace + '/io/analog_input_pos_si',
                         JointState, self.__analog_input_pos_si_callback)
        # create node
    ...
    def __foot_pedal_camera_callback(self,data):
        self.__foot_pedal_camera_state = []
        self.__foot_pedal_camera_state.append(data.buttons[0])

    def __trocar_wrench_callback(self,data):
        self.__trocar_wrench = []
        self.__trocar_wrench.append(data.force.x)
        self.__trocar_wrench.append(data.force.y)

    def __impedance_effort_callback(self, data):
        self.__impedance_effort = data.effort
        '''print(data.effort)'''
        
    def get_current_impedance_joint_effort(self):
        return self.__impedance_effort    
    ...
    def __analog_input_pos_si_callback(self, data):
        """Callback for the current joint position.

        :param data: the `JointState <http://docs.ros.org/api/sensor_msgs/html/msg/JointState.html>`_current"""
        self.__analog_input_pos_si[:] = data.position
    ...
    def get_analog_input_pos_si(self):
        """Gets the :ref:`current joint position <currentvdesired>` of the robot in terms of joint space.

        :returns: the current position of the robot in joint space
        :rtype: `JointState <http://docs.ros.org/api/sensor_msgs/html/msg/JointState.html>`_"""
        return self.__analog_input_pos_si
    ...
    def set_effort_joint(self, effort):
        ...
        self.set_effort_joint_publisher.publish(joint_state)
