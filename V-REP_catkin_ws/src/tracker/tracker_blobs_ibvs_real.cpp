...
void SelectManuallyNonDetectedBlobs (std::list<vpDot2> &blob_list, vpROSGrabber &g, vpImage<unsigned char> &I)
{
    ...
    while (blob_list.size()<4){
        g.acquire(I);
        cv::Mat I_cv;
		vpImageConvert::convert(I, I_cv);
		flip(I_cv, I_cv, +1);
		vpImageConvert::convert(I_cv, I);
        vpDisplay::display(I);
...
{
  std::ofstream image_plane_file;
  image_plane_file.open ("image_plane.txt");
  std::ofstream camera_velocity_file;
  camera_velocity_file.open ("camera_velocity.txt");
  ...
  //std::cout << a << std:: endl;
    ...
    try {
      //Initiate an image viewer
        ...
        point1.push_back( vpPoint( 0.03, +0.02, 0) );
        point1.push_back( vpPoint( 0.03, -0.02, 0) );
        /*point1[0].set_x(-0.2276265303);
        point1[0].set_y(-0.3816677187);
        point1[1].set_x(-0.388650436);
        point1[1].set_y(-0.07446480821);
        point1[2].set_x(0.2724476069);
        point1[2].set_y(0.121790139);
        point1[3].set_x(0.3122106064);
        point1[3].set_y(-0.2170592152);*/

        point1[0].set_x(-0.1783979269);
        point1[0].set_y(-0.1162519167);
        point1[1].set_x(-0.199658871);
        point1[1].set_y(0.120781529);
        point1[2].set_x(0.2146753327);
        point1[2].set_y(0.1210959205);
        point1[3].set_x(0.1922113306);
        point1[3].set_y(-0.1160214734);
        ...
        for (unsigned int i = 0; i < 4; i++) {
          vpFeatureBuilder::create(pd[i], point1[i]);
          vpFeatureBuilder::create(p[i], point1[i]);
        ...
        ros::Publisher ecm_cartesian_pub_1 = n_1.advertise<geometry_msgs::Twist>("dvrk/ECM/cartesian_velocity_desired", 1);
        ...
        vpHomogeneousMatrix cdMo(0.0, 0.0, 0.08, 0, 0, 0);
        ...
        //for (unsigned int i = 0; i < 4; i++)
          //externalview.insert(point1[i]);
#endif

        bool flag = 1;

        vpHomogeneousMatrix cextMo(0, 0, 0.3, 0, 0, 0);
        while(!vpDisplay::getClick(I, false) && ros::ok()){
          try {
            ...
            for (unsigned int i = 0; i < 4; i++) {
                vpFeatureBuilder::create(p[i], cam, dot_object_1[i].getCog());
                std::cout<< p[i].get_x() << ' ' << p[i].get_y() << std::endl;
                image_plane_file << p[i].get_x() << ' ' << p[i].get_y() << std::endl;
            }
            ...
            std::cout << "computed velocity is: \n" << v << std::endl;
            std::cout << "computed error is: \n" << e << std::endl;
            std::cout << "object 3D position: \n" << cMo_1 << std::endl;
            camera_velocity_file << v[0] << ' ' << v[1] << ' '<< v[2] << ' '<< v[3] << ' '<< v[4] << ' '<< v[5] << std::endl;
   ...
   //          for (unsigned int i=0; i < dot_object_1.size(); i ++) {
            ...
            display_trajectory(Iint, point1, cMo_1, cam);
            vpServoDisplay::display(task, cam, Iint, vpColor::green, vpColor::red,3);
#if defined(VISP_HAVE_DISPLAY)
            //externalview.display(Iext, cextMo, cMo_1, cam, vpColor::red, true);
            externalview.displayCamera(Iext, cextMo, cMo_1, cam);
#endif
